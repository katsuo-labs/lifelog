...
### 自動バックアップ・端末間同期(GitHub Gist)

設定タブで一度トークンを設定すると、記録のたびに自動で非公開Gistへバックアップされ、アプリを開いたときに他端末の記録と自動で統合(マージ同期)されます。PCとスマホの両方に同じトークンを設定すれば、どちらで記録しても揃います。
...
- 同期は記録ID単位のマージなので、両端末で同時期に記録しても互いの記録は消えません。削除も削除履歴(tombstone、180日保持)で他端末へ伝わります
- 機種変更時: 新端末で同じトークンを設定して「有効化」するだけで全データが取り込まれます
